...
evalErr (Div e1 e2) env = do {y <- evalErr e1 env ; x <- evalErr e2 env ;
                          if x == 0 then Left EDivZero else return (div y x)}
evalErr (Pow e1 e2) env = do {y <- evalErr e1 env ; x <- evalErr e2 env ;
                          if x < 0 then Left ENegPower else return (y ^ x)}
evalErr (If test yes no) env = do {x <- evalErr test env ;
